...
public abstract class Eventsender<ListenerType extends EventListener, EventType extends DefaultEvent> {
     ...
     * @param listener
     */
    public void addAllListener(final List<ListenerType> listener) {
        this.addAllListener(listener, false);
    }

    public void addAllListener(final List<ListenerType> listener, final boolean weak) {
        ...
        synchronized (this.LOCK) {
            ...
            if (weak == false) {
                ...
                if (!strongListeners.contains(t)) {
                    final List<ListenerType> newStrongListener = new ArrayList<ListenerType>(this.strongListeners);
    ...
    public boolean containsListener(final ListenerType t) {
        ...
        ListenerType l = null;
        final List<WeakReference<ListenerType>> listeners = this.weakListener;
    ...
    public List<ListenerType> getListener() {
        ...
        final List<ListenerType> ret = new ArrayList<ListenerType>(listeners.size());
        ListenerType t = null;
        for (final WeakReference<ListenerType> listener : listeners) {
            t = listener.get();
            if (t != null) {
