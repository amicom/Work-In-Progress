...
public class HTTPConnectionImpl implements HTTPConnection {
    ...
    public HTTPConnectionImpl(final URL url, final HTTPProxy p) {
        ...
        final String httpPath = new Regex(this.httpURL.toString(), "https?://.*?(/.+)").getMatch(0);
        if (httpPath == null) {
            this.httpPath = "/";
        } else {
    ...
    protected boolean putKeepAliveSocket(final Socket socket) throws IOException {
         ...
         *
         */
        if (socket != null && this.isKeepAlivedEnabled() && this.isKeepAliveOK() && socket.isConnected() && !socket.isClosed() && !socket.isInputShutdown() && !socket.isOutputShutdown()) {
        ...
        synchronized (HTTPConnectionImpl.LOCK) {
            ...
            if (sockets != null) {
                ...
                while (it.hasNext()) {
                    ...
                    } else if (!next.isSsl() && !ssl && next.sameRemoteIPs(this.remoteIPs)) {
                        it.remove();
                        HTTPConnectionImpl.KEEPALIVESOCKETS.put(socket, next);
                        return socket;
                    }
            ...
            if (this.connectionSocket == null) {
                ...
                for (final InetAddress host : this.remoteIPs) {
                    ...
                    try {
                        ...
                        if (!sslSNIWorkAround && e.getMessage().contains("unrecognized_name")) {
